jQuery(function ($) {
    ...
    function extract_features(form) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                alert("Internal CMS error: failed to create an exam!");    // can't yet rely on $.ajaxError
    ...
    function extract_features_success($form, data) {
        ...
        var $indicator = $('#badgeCreateStudent');
        $indicator.text("成功创建");
        window.location.href = "/writing/dashboard/exam-list/"
    ...
    function reset_extract_features_form($form) {
        $($form[0].elements['ExamTitle']).val('');
        $($form[0].elements['ExamTime']).val('');
        $($form[0].elements['TextareaExamDiscription']).val('');
    }
